    ...
    <aside className="hidden px-10 bg-slate-100 md:flex flex-col justify-between py-10">
      <div className="w-full flex flex-col gap-8">
        ...
        <nav className="flex flex-col gap-4">
          {
            links.map(link => (
              <Link key={link.id} href={link.link} className={`w-fit bg-slate-200 font-medium px-5 py-0.5 text-start rounded-md text-md transition hover:scale-105 ${pathname === link.link ? "bg-slate-400" : ""}`}>{link.content}</Link>
            ))
          }
        </nav>
      </div>
      <Link href="/" className="text-md hover:scale-105 transition">Cerrar sesión</Link>
    </aside>
  )
}
